    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-slate-100 p-6 relative">
                ...
                <form method="GET" action="{{ route('pekerjaan.index') }}" class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                    <div class="md:col-span-5">
                        <label for="search" class="block text-sm font-medium text-slate-700 mb-1">Nama Pekerjaan</label>
                        <input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="Ketik nama pekerjaan..." 
                    ...
                    <div class="md:col-span-3 flex justify-end space-x-2">
                        ...
                        <a href="{{ route('pekerjaan.index') }}" class="bg-rose-600 hover:bg-rose-700 text-white font-semibold py-2 px-6 rounded-xl text-sm transition duration-150 shadow-sm flex items-center justify-center">
                            Reset
                        </a>
                    </div>
                </form>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-slate-100 p-6">
                <div class="flex flex-row items-center justify-between mb-6 gap-4">
                    <div class="text-left">
                        <h3 class="text-lg font-bold text-slate-800">Tabel Master Pekerjaan</h3>
                        <p class="text-xs text-slate-500 hidden sm:block">Daftar pekerjaan aktif dan tidak aktif untuk pengelompokan pendaftaran/donor.</p>
                    </div>
                    <div>
                        <a href="{{ route('pekerjaan.create') }}" class="inline-flex items-center bg-green-600 hover:bg-green-700 text-white font-semibold py-2.5 px-4 rounded-xl text-sm transition duration-150 shadow-sm whitespace-nowrap">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="overflow-x-auto rounded-xl border border-slate-100 p-1">
                    <table id="pekerjaanTable" class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50 border-b border-slate-100">
                                <th class="py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider w-16 text-center">No</th>
                                <th class="py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Nama Pekerjaan</th>
                                ...
                                <tr class="hover:bg-slate-50/50 transition duration-150">
                                    ...
                                    <td class="py-4 px-4 text-center">
                                        <div class="flex items-center justify-center space-x-2">
                                            <!-- Detail Button -->
                                            <button onclick="showDetail('{{ addslashes($pekerjaan->pekerjaan_nama) }}', '{{ $pekerjaan->pekerjaan_aktif ? 'Aktif' : 'Tidak Aktif' }}')" 
                                            ...
                                            <!-- Edit Button -->
                                            <a href="{{ route('pekerjaan.edit', $pekerjaan->pekerjaan_id) }}" 
                                            ...
                                            <!-- Delete Button -->
                                            <form id="delete-form-{{ $pekerjaan->pekerjaan_id }}" action="{{ route('pekerjaan.destroy', $pekerjaan->pekerjaan_id) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" onclick="confirmDelete({{ $pekerjaan->pekerjaan_id }})" 
    ...
    <script>
        $(document).ready(function() {
            var table = $('#pekerjaanTable').DataTable({
                ...
                "language": {
                    ...
                    "emptyTable": "Tidak ada data pekerjaan yang ditemukan."
                }
            });
        });

        function showDetail(nama, status) {
            Swal.fire({
                title: 'Detail Pekerjaan',
                html: `
                    <div class="text-left space-y-3 text-sm">
                        <div>
                            <span class="font-semibold text-slate-500 block">Nama Pekerjaan:</span>
        ...
        function confirmDelete(id) {
            Swal.fire({
                ...
                text: "Data pekerjaan ini akan dihapus secara permanen dari sistem!",
                icon: 'warning',
