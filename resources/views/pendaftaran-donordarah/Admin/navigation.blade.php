<aside x-data="{ sidebarOpen: true, masterOpen: {{ request()->routeIs('kuesioner.*') || request()->routeIs('ruangan.*') || request()->routeIs('pekerjaan.*') ? 'true' : 'false' }}, informasiOpen: {{ request()->routeIs('admin.data-donor') ? 'true' : 'false' }} }"
    ...
    <div class="p-4 flex items-center justify-between border-b border-slate-800/60 min-h-[73px]">
        ...
        <div class="flex items-center space-x-3 overflow-hidden" x-show="sidebarOpen"
            ...
            <div
                class="w-10 h-10 rounded-lg border border-rose-500/20 flex items-center justify-center bg-rose-500/10 overflow-hidden shrink-0">
                <!-- Person Icon -->
                <svg class="w-6 h-6 text-rose-400" fill="currentColor" viewBox="0 0 24 24">
                    <path fill-rule="evenodd"
                        d="M7.5 6a4.5 4.5 0 119 0 4.5 4.5 0 01-9 0zM18.603 17.103A9 9 0 006 21h12a9 9 0 00.603-3.897z"
                        clip-rule="evenodd" />
                </svg>
            </div>
            <div class="truncate">
        ...
        <div x-show="!sidebarOpen"
            class="w-10 h-10 rounded-lg border border-rose-500/20 flex items-center justify-center bg-rose-500/10 overflow-hidden mx-auto shrink-0">
            <svg class="w-6 h-6 text-rose-400" fill="currentColor" viewBox="0 0 24 24">
                <path fill-rule="evenodd"
                    d="M7.5 6a4.5 4.5 0 119 0 4.5 4.5 0 01-9 0zM18.603 17.103A9 9 0 006 21h12a9 9 0 00.603-3.897z"
                    clip-rule="evenodd" />
        ...
        <button @click="sidebarOpen = !sidebarOpen"
            class="p-1.5 rounded-md hover:bg-slate-800 text-slate-400 hover:text-slate-200 focus:outline-none shrink-0 ml-auto">
    ...
    <div class="flex-1 py-4 overflow-y-auto px-3 space-y-1">
        ...
        <a href="{{ route('dashboard') }}"
            ...
            <svg class="w-5 h-5 mr-3 shrink-0 {{ request()->routeIs('dashboard') ? 'text-rose-400' : 'text-slate-400' }}"
            ...
            <button @click="masterOpen = !masterOpen"
                ...
                <div class="flex items-center">
                    ...
                    <svg class="w-5 h-5 mr-3 text-slate-400 shrink-0" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M4 7v10c0 2 1.5 3 3.5 3h9c2 0 3.5-1 3.5-3V7M4 7c0-2 1.5-3 3.5-3h9c2 0 3.5 1 3.5 3M4 7h16" />
                    </svg>
                    <span x-show="sidebarOpen" x-transition:enter="transition ease-out duration-200"
                        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">Master</span>
                </div>
                <svg x-show="sidebarOpen"
                    class="w-4 h-4 text-slate-400 transform transition-transform duration-200 shrink-0"
                    :class="masterOpen ? 'rotate-90' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                </svg>
            </button>

            <!-- Submenu Master -->
            <div x-show="masterOpen && sidebarOpen" x-transition:enter="transition ease-out duration-200"
        ...
        <div>
            <button @click="informasiOpen = !informasiOpen"
                ...
                <div class="flex items-center">
                    ...
                    <span x-show="sidebarOpen" x-transition:enter="transition ease-out duration-200"
                        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">Informasi</span>
                </div>
                <svg x-show="sidebarOpen"
                    class="w-4 h-4 text-slate-400 transform transition-transform duration-200 shrink-0"
                    :class="informasiOpen ? 'rotate-90' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                </svg>
            </button>
    ...
    <div class="p-4 border-t border-slate-800/60 space-y-2">
        ...
        <a href="{{ route('profile.edit') }}"
            ...
            <span x-show="sidebarOpen" x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">Setting</span>
        </a>
        ...
        <form method="POST" action="{{ route('logout') }}">
            ...
            <button type="submit"
                ...
                <svg class="w-5 h-5 mr-3 text-rose-400 shrink-0" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
